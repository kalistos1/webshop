...
def guestOrder(request, data):
	...
	# a list for ordered items and quantity	
	item_title = []
	item_quantity = []
	...
	for item in items:
		...
		orderItem = CartOrderItem.objects.create(
			product=product,
			order=order,
		...
		# stringifying the items and quantity
		product=str(product)
		quantity= str(item['quantity'])
       
	    # appending the items and quantity to thier respective lists
		item_title.append(product)
		item_quantity.append(quantity)
	return customer, order, orderItem, item_title, item_quantity
